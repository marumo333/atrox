...
| 機能 | 説明 | 優先度 |
|------|------|--------|
| キャラクター作成 | Vesperの人格をベースに自分専用キャラを設定 | P0 |
| プライベート連載 | 自分だけのために毎週話が生成される | P0 |
| スタイルカスタマイズ | ジャンル・トロープ・文体を調整可能 | P1 |
| キャラクター記憶 | ユーザー専用のarc_stateを永続管理 | P0 |
...
- Stripe手数料：2.9% + $0.30（海外カード）。デジタルコンテンツは返金不可ポリシーを適用
...
| StripeのコンテンツポリシーNG | 低 | 高 | Claude APIのコンテンツポリシー範囲内に収める |
